...
local in_language_block = require("util.snippets").in_language_block
...
local function in_sql_block()
  return in_language_block({ "sql", "SQL", "PostgreSQL" })
end
...
local snippets = {
  ...
  s("sum(", t("SUM(")),
  s("count(", t("COUNT(")),
  s({ trig = "group b", priority = 1001 }, t("GROUP B")),
  s({ trig = "GROUP By", priority = 1001 }, t("GROUP BY")),
  s("having", t("HAVING")),
  s("union", t("UNION")),
...
for _, snip in pairs(snippets) do
  snip["condition"] = in_sql_block
end

-- ls.add_snippets('markdown', snippets, { type = "autosnippets", key = "sql_auto" })
